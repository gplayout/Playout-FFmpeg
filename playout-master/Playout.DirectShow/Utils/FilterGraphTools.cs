...
namespace Playout.DirectShow.Utils
{
    public sealed class FilterGraphTools
    {
        ...
        [SecurityPermission(SecurityAction.LinkDemand, UnmanagedCode = true)]
        public static void SaveFilterProperties(IBaseFilter filter, string fileName)
        {
            int hr = 0;
            IStorage storage = null;
            IStream stream = null;
            ...
            try
            {
                ...
                hr = (filter as IPersistStream).Save(stream, true);
                Marshal.ThrowExceptionForHR(hr);
        ...
        [SecurityPermission(SecurityAction.LinkDemand, UnmanagedCode = true)]
        public static void LoadFilterProperties(IBaseFilter filter, string fileName)
        {
            int hr = 0;
            IStorage storage = null;
            IStream stream = null;
            try
            {
                ...
                hr = (filter as IPersistStream).Load(stream);
                Marshal.ThrowExceptionForHR(hr);
            }
            finally
            {
