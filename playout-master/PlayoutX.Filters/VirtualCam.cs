...
namespace PlayoutX.Filters
{
    ...
    public class VideoData
    {
        ...
        public Bitmap BitmapFrame { get; set; }
    }
    ...
    public class VirtualCamFilter : BaseSourceFilter, IAMFilterMiscFlags
    {
        ...
        private int c_iDefaultWidth = 1024;
        private int c_iDefaultHeight = 768;
        ...
        private const int c_nMaxWidth = 1920;//c_nMinWidth + c_nGranularityW * (c_iFormatsCount - 1);
        private const int c_nMaxHeight = 1080;//c_nMinHeight + c_nGranularityH * (c_iFormatsCount - 1);
        ...
        protected int m_nWidth = 1024;
        protected int m_nHeight = 768;
        ...
        protected long lastFrameIndex = -1;
        ...
        private VideoData ReadBitmapfromPipe()
        {
            try
            {
                if (mmf == null)
                    mmf = MemoryMappedFile.OpenExisting("PlayoutXVCam");
                //
                Mutex mutex = Mutex.OpenExisting("PlayoutXVCammutex");
                while(true)
                {
                    ...
                    using (MemoryMappedViewStream stream = mmf.CreateViewStream())
                    {
                        ...
                        vd.FrameIndex = BitConverter.ToInt64(buffer, 0);
                        //
                        if (this.lastFrameIndex == vd.FrameIndex)
                        {
                            mutex.ReleaseMutex();
                            Thread.Sleep(5);
                        ...
                        //
                        checkBufferCount = 0;
                        lastFrameIndex = vd.FrameIndex;
                        //
                        vd.FrameRate = BitConverter.ToInt32(buffer, 8);
                        //
                        using (var ms = new MemoryStream(buffer.Skip(20).ToArray()))
                        {
                            if (m_hBitmap != IntPtr.Zero)
                            {
                                DeleteObject(m_hBitmap);
                            ...
                            //
                            vd.BitmapFrame = (Bitmap)Bitmap.FromStream(ms);
                        }
                        //
                        mutex.ReleaseMutex();
                        return vd;
        ...
        private void GetBitmapInfo(out int width, out int height, out int frameRate)
        {
            width = 0; height = 0; frameRate = 0;
            try
            {
                VideoData vd = this.ReadBitmapfromPipe();
                frameRate = vd.FrameRate;
                width = vd.BitmapFrame.Width;
                height = vd.BitmapFrame.Height;
            }
            catch (Exception)
            {
            }
        ...
        private IntPtr CreateBitmap(int width, int height)
        {
            ...
            try
            {
                VideoData vd = this.ReadBitmapfromPipe();
                //
                vd.BitmapFrame.RotateFlip(RotateFlipType.RotateNoneFlipY);
                vd.BitmapFrame = this.ResizeImage(vd.BitmapFrame, new Size(width, height));
                result = vd.BitmapFrame.GetHbitmap();
                //
                return result;
            }
        ...
        public int FillBuffer(ref IMediaSampleImpl _sample)
        {
            try
            {
                m_hBitmap = this.CreateBitmap(m_nWidth, Math.Abs(m_nHeight));
                ...
                //
                //Marshal.FreeHGlobal(m_hBitmap);
                //Marshal.FreeHGlobal(hOldBitmap);
                return NOERROR;
            }
            catch
            {
        ...
        public int GetDefaultCaps(int nIndex, out VideoStreamConfigCaps _caps)
        {
            ...
            if (!readBitmapInfo)
            {
                ...
                int frameRate=0;
                this.GetBitmapInfo(out width, out height, out frameRate);
                if(frameRate!=0)
                {
                    this.m_nAvgTimePerFrame = frameRate;
                }
                if (width != 0 && height != 0)
                {
                    c_iDefaultWidth = width;
                    c_iDefaultHeight = height;
                }
                readBitmapInfo = true;
            }
            //
